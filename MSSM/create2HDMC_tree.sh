...
function createOneTree() {
  ...
  allJobs=$((`ls $dir/MSSM_mhmax*_SuShi.out | wc -l `))
  echo Working on dir $dir on $allJobs points
  outFile=$dir/parameters_2HDMC.data
  mv -f ${outFile} ${outFile}_save
  touch ${outFile}
  njob=0
  for f in `ls ${dir}/MSSM_mhmax_mA225_tb1p00_SuShi.out`
  do
    njob=$((njob+1))
