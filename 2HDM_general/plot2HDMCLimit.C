...
void plot2HDMCLimit(int m=300, int type=2, const TString& fileLimit="Int2D_M300.txt") {
  ...
  hggA->GetXaxis()->SetRangeUser(-1.,1.);
  TString cut=what+ok;
  ...
  // for (int jj=0 ; jj<hggA->GetNbinsY(); ++jj) {
  //   //hggA->SetBinContent(90,jj,100.);
  //  // hggA->SetBinContent(100,jj,hggA->GetBinContent(99,jj)*2.);
  //   //hggA->SetBinContent(101,jj,hggA->GetBinContent(99,jj)*10.);
  // }
...
//
  ...
  Color_t col[6]={kYellow,kGreen, kBlack, kYellow, kWhite };
  Int_t lstyle[5]={1,1,2,1,1};
  Int_t lwidth[5]={2,2,2,2,2};
  ...
  for(i = 0; i < TotalConts; i++){
    ...
    for(int j = 0; j < contLevel->GetSize(); j++){
      ...
      if (i!=2) curv->SetFillColor(col[i]);
      ...
      if(i!=2) { // complete the area polygons with points at the plot edges

        //cout << i << "x: " << gc->GetX()[0] << " "  << gc->GetX()[gc->GetN()]  << " y: " << gc->GetY()[0] << " " << gc->GetY()[gc->GetN()] << endl;
        // left side
        if(gc->GetX()[0]<0 ) {
          // if it is an island: fill with white
          if(gc->GetX()[0]>-0.97 && gc->GetY()[0]>0.011 && gc->GetY()[0]<98 ) { // Left side
            gc->SetFillColor(kWhite);
          }
          // else, add a point outside
          else {
            //cout << "Adding a point left" << endl;
            gc->SetPoint(gc->GetN(), -1.2, gc->GetX()[0]);
          }
        }
        // right side
        else if (type==2 && gc->GetX()[0]>0 ){
          // if it is an island: fill with white
          if(fabs(gc->GetX()[0]-gc->GetX()[gc->GetN()])>0.1  ) { // Left side
            //cout << "Island" << endl;
            gc->SetFillColor(kWhite);
          }
          // else, add a point outside
          else {
            // for (jj=0; jj<gc->GetN(); ++jj) {
            //   cout << jj << " " << gc->GetX()[jj] << " " << gc->GetY()[jj]<< endl;
            // }
            // cout << "Adding a point right" << endl;
            gc->SetPoint(gc->GetN(),0.4,300);
            gc->SetPoint(gc->GetN(),1.2,300);
            gc->SetPoint(gc->GetN(),1.2,0.1);
            //gc->SetPoint(gc->GetN(),10,5);
            gc->SetPoint(gc->GetN(),0.1,0.1);
            gc->SetPoint(gc->GetN(),gc->GetX()[0],gc->GetY()[0]);
            // for (jj=0; jj<gc->GetN(); ++jj) {
            //   cout << jj << " " << gc->GetX()[jj] << " " << gc->GetY()[jj]<< endl;
            // }
          }
